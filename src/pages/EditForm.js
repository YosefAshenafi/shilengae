...
import { deleteField } from '../request/formFields';
...
export default function EditForm() {
  ...
  useEffect(() => {
    getFormDetail(id).then((res) => {
      ...
      setFieldList(res.form_fields.map((field) => ({ ...field, saved: true })));
    });
  }, [id]);
    ...
    <Page title="Edit Form | Shilengae">
      <Container>
        ...
        <Button
          ...
          onClick={() => {
            setIsLoading(true);
            updateForm(id, { name: formName, form_fields: fieldList })
              .then(() => {
